...
Integrate GoJira with Dashforge to provide a powerful, reusable report generation system that supports both interactive notebook-style exploration and static report export. This integration enables users to create Jira-based reports and dashboards using a JSON-first intermediate representation (IR) that can be rendered interactively or exported to static formats.

## Problem Statement

Currently, GoJira provides excellent CLI and SDK capabilities for querying Jira data and generating reports (velocity, burndown, worklog, cycle-time). However:
...
- **GoJira** handles Jira data fetching, transformation, and business logic
...
- Execute JQL queries via GoJira SDK
...
- GoJira SDK v0.37+ compatibility
...
1. **Adoption**: 50% of GoJira users try report generation within 3 months
